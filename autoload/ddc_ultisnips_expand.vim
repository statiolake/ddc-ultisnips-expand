let s:TextEdit = vital#ddc_ultisnips_expand#import('VS.LSP.TextEdit')
let s:Position = vital#ddc_ultisnips_expand#import('VS.LSP.Position')
...
inoremap <silent> <Plug>(ddc_ultisnips_expand:on_complete_done_after) <C-r>=<SID>on_complete_done_after()<CR>

"
" ddc_ultisnips_expand#skip_complete_done_after
"
function! ddc_ultisnips_expand#skip_complete_done_after() abort
  let s:stop_complete_done_after = v:true
  call timer_start(0, { -> execute('let s:stop_complete_done_after = v:false') })
endfunction

"
" ddc_ultisnips_expand#do_complete_done
"
" @param context = {
...
" }
"
function! ddc_ultisnips_expand#do_complete_done(context) abort
  ...
  \ }
  call feedkeys("\<Plug>(ddc_ultisnips_expand:on_complete_done_after)")
endfunction

"
" ddc_ultisnips_expand#on_complete_done
"
function! ddc_ultisnips_expand#on_complete_done(completed_item) abort
  let l:context = s:extract_user_data(a:completed_item)
  if !empty(l:context)
    if s:stop_complete_done | return | endif

    if l:context['sources'] == ["ultisnips"]
      " ddc-ultisnips: just expand snippet
...
function! s:on_complete_done_after() abort
  ...
  " Expand snippet.
  if strlen(l:expand_text) > 0
    call UltiSnips#Anon(l:expand_text)
  endif
...
function! s:apply_additional_text_edits(context) abort
  ...
  for l:id in ['lcn', 'vimlsp']
    if index(s:context.sources, l:id) >= 0 && ddc_ultisnips_expand#detection#exists(l:id)
      return
    endif
  endfor
...
function! s:extract_user_data(completed_item) abort
  try
    ...
    if s:has_key(l:user_data, 'ultisnips')
      " ddc-ultisnips
      return {
      \   'sources': ['ultisnips'],
      \ }
    ...
    if g:ddc_ultisnips_expand_debug
